...
| 窗口 | 展开/缩小切换 | 完整面板为 320×320，缩略窗口为 100×100；按窗口最终宽度直接切换两套实时界面；Windows 在同一次原生窗口事务中提交尺寸和右下角位置并保存状态 | Rust 几何测试覆盖 100%/125%/150% 与负坐标显示器；前端状态测试、浏览器 320/100 像素边界与 100% 三位数检查、Windows Tauri 无安装包构建通过；待 Windows 实机逐帧与 macOS 验证 |
| 托盘 | Windows 托盘菜单 | 面板显示设置可持久保存；隐藏后可由托盘图标恢复；刷新、解锁、语言切换、开机启动、退出可用 | 待 Windows 实机验证 |
| 菜单栏 | macOS 菜单栏托盘 | 面板显示设置可持久保存；隐藏后可由菜单栏图标恢复；刷新、解锁、语言切换、开机启动、退出可用 | 待 macOS 实机验证 |
